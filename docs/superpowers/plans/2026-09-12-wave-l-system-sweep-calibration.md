...
- [ ] **Step 2: Failing test** `corpus_test.go`: (a) `TestCorpusVendoringSelfConsistent` — walk testdata dirs via `os.ReadDir` (tests may read their package's testdata directly; unembedded); every file's sha256 equals the VENDOR.json record; VENDOR lists exactly the 8 targets; no unlisted files at either level. (b) `TestCorpusExpectationsConform` — per expected.json (AMENDED post-landing to the real corpus): `status` in {expected, unwritable}, unwritable envelopes must carry a closed-set `expected_error_code`; verdict in {PROVEN,VIOLATED,UNKNOWN}; `expected_reason_code` null or `harness.IsReasonCode` true (empty string rejected, stricter than first draft); `solc` == 0.8.36 pinned literal; tool_flags contains --loop-bound; invariant-cap pins rule_id `cap_respected` (the real declaration name in cap.mspec) with its PROVEN literals written verbatim into the test (per_function deposit/setCap, init proved, witness_function null, 5 assumptions — see commit 31e4bea1). (c) `TestReasonCodeSetIsTwentyFive` — the 25-name hard slice (from runner.py) all true, `nope-code` false, and len(dispositionOf) == 25.
