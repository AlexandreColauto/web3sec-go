...
**Files:** Modify `internal/harness/witness.go` only (BridgeSequence emits `final_assertions` from the report's `final_storage` where values are concrete literals: `{"id":"A1","kind":"storage","target":<contract addr or refusal>,"account"?...}` — READ the schema's storage assertion keys EXACTLY (assets/schema/sequence_poc.schema.json final_assertions items: id/kind/op/value + target/account/slot fields — copy the real required set); ids A1..An in sorted-slot order; symbolic/pretty values → that assertion SKIPPED (and if ANY skipped, keep the emitted subset honest: docstring line `n of m`); op is `==` (observed final value), values wei/hex-literal pattern per schema.
