...
```bash
webv2 audit <C-xxx> [--json]     # full integrity audit (event-log chain, artifacts, execs, findings, projection, relations, floor policy, snapshots — including a named active pin whose directory is gone — plus, once the campaign prices anything, the price_table reconciliation of prices.json against the logged price.set decisions; a hand-edited USD figure is drift; and, always last, v16_coverage — the v1.6 record-coverage section, which counts all eight new fields (input_artifacts on model.request events, review_sessions, deployment_facts, poc_tier, replay, fork_dependence, origin_stage, contributing_stages) and raises problems only for the three impossible states: a maximized tier with no existence tier under it, an unprofitable replay with no recorded blocker, and a deployment fact read at no pinned block)
