...
# 4. the audit: ok + all 16 rendered sections (19 registered; `eval`,
#    `price_table` and `exec_record_anchor` are presence-gated, `v16_coverage`
#    is unconditional; the anchor renders because this campaign ran real
#    execs) + the VACUOUS sequence_coverage pin
...
if len(secs) != 16:
    fails.append(f"{len(secs)} audit sections, want 16")
