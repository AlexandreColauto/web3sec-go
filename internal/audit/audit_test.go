...
func TestAuditCleanCampaignPasses(t *testing.T) {
	...
	want := []string{"event_log", "artifacts", "execs", "findings", "projection", "snapshots"}
...
// TestAuditRegistryListsSixInOrder: the section registry exposes exactly
// the six P0 names in deterministic (report) order.
func TestAuditRegistryListsSixInOrder(t *testing.T) {
	Setup()
	want := []string{"event_log", "artifacts", "execs", "findings", "projection", "snapshots"}
