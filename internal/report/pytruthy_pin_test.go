...
// pyTruthyInt64Only pin (Wave J Task 7): records the truthiness this definition was
// already computing BEFORE the consolidation, so the unify/rename step cannot
// move a gate. Variant: pyTruthyInt64Only — Int truthiness reads only the int64 field I; a big integer (Big set) reads false.
...
func TestPyTruthyPin(t *testing.T) {
	cases := []struct {
		...
		{"big_huge", validation.VBigInt("123456789012345678901234567890"), false},
		{"big_zero_literal", validation.VBigInt("0"), false},
	}
	for _, tc := range cases {
		if got := pyTruthyInt64Only(tc.v); got != tc.want {
			t.Errorf("pyTruthyInt64Only(%s) = %v, want %v", tc.name, got, tc.want)
		}
	}
}
