...
func TestDivergenceStatusPreA3Oracle(t *testing.T) {
	...
	for _, c := range cases.A {
		got := DivergenceStatus(validation.ObjAt(c, "plan"), DivergenceOpts{})
		requireJSON(t, "divergence/"+validation.ObjStr(c, "name"), got, validation.ObjAt(c, "out"))
	}
}
...
func TestDivergenceStatusA3Oracle(t *testing.T) {
	...
	for _, c := range cases.A {
		surface := validation.ObjAt(c, "surface")
		withProbes(t, probeEnv{surface: &surface, sha: shaPtr(c),
			blanks: blanksOf(c)})
		got := DivergenceStatus(validation.ObjAt(c, "plan"), DivergenceOpts{
			Surface: &surface, CurrentIndexSha: shaPtr(c),
			Blanks: blanksOf(c)})
		requireJSON(t, "a3/"+validation.ObjStr(c, "name"), got, validation.ObjAt(c, "out"))
		closure := LensProbeClosure(validation.ObjAt(c, "plan"), "L-03",
			DivergenceOpts{Surface: &surface, CurrentIndexSha: shaPtr(c),
				Blanks: blanksOf(c)})
		if closure == nil {
			t.Fatalf("a3/%s: closure is nil", validation.ObjStr(c, "name"))
		}
		requireJSON(t, "a3 closure/"+validation.ObjStr(c, "name"), *closure,
			validation.ObjAt(c, "closure"))
	}
}

// shaPtr is the case's current index sha (null = no index).
func shaPtr(c validation.Value) *string {
	if s := validation.ObjAt(c, "sha"); s.Kind == validation.Str {
		return &s.S
	}
	return nil
}
...
func TestProbeMissingHintNamesCampaign(t *testing.T) {
	...
	joined := ""
	for _, m := range got {
		joined += validation.ObjStr(m, "what") + "\n"
	}
...
func TestProbeMissingOnlyOracle(t *testing.T) {
	...
	closed := validation.ObjAt(a3.A[0], "plan")
	surface := validation.ObjAt(a3.A[0], "surface")
	sha := validation.ObjStr(a3.A[0], "sha")
	...
	trimmed := validation.ObjAt(a3.A[4], "surface")
...
func TestDivergenceStatusForReadsCampaign(t *testing.T) {
	...
	c := at(t, root, "a3").A[0]
	surface := validation.ObjAt(c, "surface")
	sha := validation.ObjAt(c, "sha")
	withProbes(t, probeEnv{surface: &surface, sha: &sha.S})
	camp := newCampaign(t, "dsf")
	got, err := DivergenceStatusFor(camp, validation.ObjAt(c, "plan"), nil)
	if err != nil {
		t.Fatalf("divergence_status_for: %v", err)
	}
	requireJSON(t, "for", got, validation.ObjAt(c, "out"))
}
...
func TestFakeAnchorRefOracle(t *testing.T) {
	...
	for _, row := range listOf(surface, "rows") {
		want := validation.ObjAt(refs, validation.ObjStr(row, "row_id"))
		...
		for _, anchor := range want.O {
			...
			if err != nil {
				t.Fatalf("anchor_ref(%s, %s): %v", validation.ObjStr(row, "row_id"),
					anchor.K, err)
			}
			if got != anchor.V.S {
				t.Fatalf("anchor_ref(%s, %s) = %q, want %q",
					validation.ObjStr(row, "row_id"), anchor.K, got, anchor.V.S)
			}
			n++
		}
	}
...
func noClassPlan(t *testing.T) validation.Value {
	...
	for _, c := range at(t, oracles(t), "divergence").A {
		if validation.ObjStr(c, "name") == "grandfather" {
			return deepCopy(t, validation.ObjAt(c, "plan"))
		}
	}
...
func divWhat(v validation.Value, subject string) string {
	for _, m := range listOf(v, "missing") {
		if validation.ObjStr(m, "subject") == subject {
			return validation.ObjStr(m, "what")
		}
	}
	return ""
}
...
func TestDivergenceDiversityCountsCampaignFindings(t *testing.T) {
	...
	if !gateClosed(closedGate) {
		t.Fatalf("4 distinct canonical classes must close the diversity "+
			"clause; missing = %s",
			validation.CanonCompact(validation.ObjAt(closedGate, "missing")))
	}
	...
	if n := len(listOf(closedGate, "named_classes")); n != 4 {
		t.Errorf("named_classes = %s",
			validation.CanonCompact(validation.ObjAt(closedGate, "named_classes")))
	}
}
...
func TestNamedClassesUnionsCampaignClasses(t *testing.T) {
	...
	if validation.CanonCompact(validation.StrArr(got)) != validation.CanonCompact(
...
func TestDivergenceStatusForCountsFindingsClasses(t *testing.T) {
	...
	if div := divOf(); gateClosed(div) {
		...
		t.Errorf("non-canonical finding class counted: %s",
			validation.CanonCompact(validation.ObjAt(div, "named_classes")))
	}
