...
func logHypothesisRequest(campaign *state.Campaign,
	...
	if err := ValidateRequest(request); err != nil {
		...
		// (a returned error, no event): it may carry no valid role/kind at
		// all, and model.rejected's own contract
		...
		// the framework itself wrote. InputSetRecordable states the gate:
		// valid without input_artifacts, and the derived event conformant.
		if InputSetRecordable(request, err.Error()) {
