...
func classes() validation.Value {
	...
	return validation.ObjAt(doc, "classes")
}
...
func SearchFactor(class string) float64 {
	row := validation.ObjAt(classes(), class)
	if row.Kind != validation.Obj {
		return 1.0
	}
	v := validation.ObjAt(row, "search")
...
func ClassesWithNonNeutralWeights() []string {
	...
	for _, kv := range classes().O {
		if validation.ObjAt(kv.V, "search").F != 1.0 || validation.ObjAt(kv.V, "acceptance").F != 1.0 {
			out = append(out, kv.K)
		}
	}
	sort.Strings(out)
