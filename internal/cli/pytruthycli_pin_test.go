...
// move a gate. Variant: pyTruthyCLI — Int truthy when I != 0 OR Big != "" (CLI layer; out of scope for this unification).
...
func TestPyTruthyCLIPin(t *testing.T) {
	cases := []struct {
		name string
		v    validation.Value
		want bool
	...
	for _, tc := range cases {
		if got := pyTruthyCLI(tc.v); got != tc.want {
			t.Errorf("pyTruthyCLI(%s) = %v, want %v", tc.name, got, tc.want)
		}
	}
}
