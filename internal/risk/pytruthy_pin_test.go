...
// pyTruthyBigNonEmpty pin (Wave J Task 7): records the truthiness this definition was
// already computing BEFORE the consolidation, so the unify/rename step cannot
// move a gate. Variant: pyTruthyBigNonEmpty — Int truthy when I != 0 OR Big != "" (Big == "0" reads truthy).
...
func TestPyTruthyPin(t *testing.T) {
	...
	for _, tc := range cases {
		if got := pyTruthyBigNonEmpty(tc.v); got != tc.want {
			t.Errorf("pyTruthyBigNonEmpty(%s) = %v, want %v", tc.name, got, tc.want)
		}
	}
}
