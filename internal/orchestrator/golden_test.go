...
//
// Task 7 (production readiness) re-recorded the 27 next_actions oracles
// (scenarios fresh/scoped/snapped/model/planned/ingested/deduped/gates and all
// 19 steps of `phases`) from the Go twin for the same reason: the twin's
// guidance catalog printed Python-API pseudo-calls
// (`orchestrator.scope(policy_path=...)`), and the law is now that every
// next-action line is a copyable `webv2 …` command. The recorded Python bytes
// are preserved nowhere else, so these 27 snapshots are Go-authored by design;
// the re-record was generated by replaying each scenario through
