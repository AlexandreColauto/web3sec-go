...
func TestFullSubmissionReady(t *testing.T) {
	...
	if checks := objAt(result, "policy_checks"); len(checks.A) != 13 {
		t.Errorf("policy_checks = %d rows, want 13", len(checks.A))
	}
}
...
func TestEvaluateSavesFindingAndLogs(t *testing.T) {
	...
	if checks := objAt(sb, "policy_checks"); len(checks.A) != 13 {
		t.Errorf("stored policy_checks = %d rows, want 13", len(checks.A))
	}
	events, err := c.Events()
	if err != nil {
		t.Fatal(err)
...
var gateGolden = map[string]string{
	...
	"immunization_bypass":  "{\"blocking_reasons\": [\"not immunized (bypass) \\u2014 the patch must block the FORK PoC and its 3 boundary mutations\"], \"eligible\": true, \"policy_checks\": [{\"check\": \"security-confirmed\", \"detail\": \"\", \"result\": \"pass\"}, {\"check\": \"snapshot-pinned\", \"detail\": \"\", \"result\": \"pass\"}, {\"check\": \"in-scope\", \"detail\": \"matched scope entry 'Vault'\", \"result\": \"pass\"}, {\"check\": \"known-issue-check\", \"detail\": \"no exclusion pattern matched\", \"result\": \"pass\"}, {\"check\": \"severity-floor\", \"detail\": \"matched severity rule for high\", \"result\": \"pass\"}, {\"check\": \"evidence-sufficient\", \"detail\": \"E5 >= required E5\", \"result\": \"pass\"}, {\"check\": \"fork-repro\", \"detail\": \"tier T3\", \"result\": \"pass\"}, {\"check\": \"maximal-exploitation\", \"detail\": \"ladder closed (maximal: R-abc123)\", \"result\": \"pass\"}, {\"check\": \"e7-price-basis\", \"detail\": \"PRICE-0001 -> ACME @ $1.0 (fixture: fixed reference price)\", \"result\": \"pass\"}, {\"check\": \"claim-drift\", \"detail\": \"\", \"result\": \"pass\"}, {\"check\": \"precondition-audit\", \"detail\": \"\", \"result\": \"pass\"}, {\"check\": \"mainnet-fork-poc\", \"detail\": \"fork PoC proven: E5 fork-test from EXEC-0000000002 (fork-runner, exit 0)\", \"result\": \"pass\"}, {\"check\": \"immunization\", \"detail\": \"bypass: boundary bypass found: calling via delegatecall from a proxy skips the check \\u2014 the patch does not hold\", \"remediation\": \"webv2 immunize <fid> --poc-exec <FORK-EXEC-ID> --patch '<the fix>' --mutations 'm1;m2;m3'   (the patch must block the FORK PoC and all 3 boundary mutations \\u2014 a unit-test patch is not a patch; if a bypass is real, fix the patch and re-verify)\", \"result\": \"fail\"}], \"submission_ready\": false}",
}
...
var explainGolden = map[string]string{
	...
	"evidence-floor":             "{\"check\": \"evidence-floor\", \"gate\": \"confirmed\", \"remediation\": \"webv2 mint <fid> --exec <EXEC-ID>   (or, for a NAMED decision: webv2 floors set \\u2014 an override, logged, never a silent edit)\"}",
...
var bountyRemediationOrder = []string{"security-confirmed", "snapshot-pinned", "in-scope", "known-issue-check", "severity-floor", "evidence-sufficient", "fork-repro", "economic-quantified", "maximal-exploitation", "e7-price-basis", "claim-drift", "precondition-audit", "mainnet-fork-poc", "immunization"}

// ---- unknown-check error (str(KeyError(msg))) ----
const wantUnknownCheck = "\"unknown check id 'no-such-check' (known: ['claim-drift', 'critic-verdict', 'e7-price-basis', 'economic-quantified', 'evidence-floor', 'evidence-floor-unreachable', 'evidence-sufficient', 'fork-repro', 'immunization', 'in-scope', 'invariant-unverified', 'known-issue-check', 'mainnet-fork-poc', 'maximal-exploitation', 'memory-check', 'precondition-audit', 'reproduction-reproduced', 'reproduction-tier', 'security-confirmed', 'sequence-coverage', 'severity-floor', 'shield-adjudication', 'snapshot-compatible', 'snapshot-pinned'])\""
...
var confirmedRemediationGolden = map[string]string{
	...
	"evidence-floor":             "webv2 mint <fid> --exec <EXEC-ID>   (or, for a NAMED decision: webv2 floors set \u2014 an override, logged, never a silent edit)",
...
var gateVectorOrder = []string{"full_pass", "ladder_missing", "ladder_open", "not_confirmed", "ladder_waived", "no_usd_no_affected", "evidence_floor", "fork_tier", "out_of_scope", "known_issue", "economic_below_floor", "immunization_bypass"}
