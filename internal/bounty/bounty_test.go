...
func TestFullSubmissionReady(t *testing.T) {
	...
	if checks := objAt(result, "policy_checks"); len(checks.A) != 14 {
...
func TestEvaluateSavesFindingAndLogs(t *testing.T) {
	...
	if checks := objAt(sb, "policy_checks"); len(checks.A) != 14 {
...
var bountyRemediationOrder = []string{"security-confirmed", "snapshot-pinned", "in-scope", "known-issue-check", "severity-floor", "evidence-sufficient", "fork-repro", "economic-quantified", "maximal-exploitation", "e7-price-basis", "claim-drift", "precondition-audit", "mainnet-fork-poc", "immunization", "accepted-risk"}
