...
// twin: gen_go.py [untracked] emits the CanonSpaced bounty dict, the
...
// clears the 20-rune floor with an actual incentive argument. The fourth
// field (strongest_attacker) is the morph §7.2 arm — the proof-VALID bad
// state the challenge_interplay claim must survive.
func adversarialClause() validation.Value {
	return validation.VObj(
		...
		kv("strongest_attacker", validation.VStr(
			"the strongest variant is a proof-valid bad batch: the operator "+
				"posts a fake prev root and proves a valid transition FROM "+
				"it, so the challenge verifies and the freeze survives")))
}
...
func vectorAdversarialCases(cases map[string]vectorCase) {
		...
		finding: withField(livenessFinding(), "adversarial_game",
			validation.VObj(
				...
				kv("challenge_interplay", validation.ObjAt(adversarialClause(),
					"challenge_interplay")),
				kv("strongest_attacker", validation.ObjAt(adversarialClause(),
					"strongest_attacker")))),
		active: "SNAP-11111111", seams: submissionReadySeams(),
	}
	...
	// adversarial_complete: the clause answers all four questions
	// (morph §7.2 adds the strongest attacker arm) —
...
//
// morph §7.2: the four adversarial_* pins were regenerated in this commit
// because check15's complete-clause detail gained the strongest_attacker
// field (and its remediation gained --strongest-attacker). Each case's
// pass/fail semantics are unchanged — only the pinned bytes moved.
var gateGolden = map[string]string{
	...
	"adversarial_waived":     "{\"blocking_reasons\": [], \"eligible\": true, \"policy_checks\": [{\"check\": \"security-confirmed\", \"detail\": \"\", \"result\": \"pass\"}, {\"check\": \"snapshot-pinned\", \"detail\": \"\", \"result\": \"pass\"}, {\"check\": \"in-scope\", \"detail\": \"matched scope entry 'Vault'\", \"result\": \"pass\"}, {\"check\": \"known-issue-check\", \"detail\": \"no exclusion pattern matched\", \"result\": \"pass\"}, {\"check\": \"severity-floor\", \"detail\": \"matched severity rule for high\", \"result\": \"pass\"}, {\"check\": \"evidence-sufficient\", \"detail\": \"E5 >= required E5\", \"result\": \"pass\"}, {\"check\": \"fork-repro\", \"detail\": \"tier T3\", \"result\": \"pass\"}, {\"check\": \"maximal-exploitation\", \"detail\": \"ladder closed (maximal: R-abc123)\", \"result\": \"pass\"}, {\"check\": \"e7-price-basis\", \"detail\": \"PRICE-0001 -> ACME @ $1.0 (fixture: fixed reference price)\", \"result\": \"pass\"}, {\"check\": \"claim-drift\", \"detail\": \"\", \"result\": \"pass\"}, {\"check\": \"precondition-audit\", \"detail\": \"\", \"result\": \"pass\"}, {\"check\": \"mainnet-fork-poc\", \"detail\": \"fork PoC proven: E5 fork-test from EXEC-0000000002 (fork-runner, exit 0)\", \"result\": \"pass\"}, {\"check\": \"immunization\", \"detail\": \"patch blocks the fork PoC and all 3 boundary mutations (basis: EXEC-0000000002)\", \"result\": \"pass\"}, {\"check\": \"accepted-risk\", \"detail\": \"no accepted-risk pattern matched\", \"result\": \"pass\"}, {\"check\": \"paid-exploitability\", \"detail\": \"paid \\u2014 argument recorded (535 chars)\", \"result\": \"pass\"}, {\"check\": \"adversarial-game\", \"detail\": \"liveness finding is missing the adversarial_game clause\", \"remediation\": \"webv2 adversarial-game <campaign> <fid> --who-profit 'who profits from the freeze' --mechanism 'how the profit works' --interplay 'why the challenge path does not undo it' --strongest-attacker 'does a proof-valid bad batch still win the challenge?'   (each field >= 20 chars; or: webv2 waive <campaign> adversarial-game --subject <fid> --reason '...' if the incentive argument lives elsewhere, e.g. the chain narrative)\", \"result\": \"fail\"}, {\"check\": \"adversarial-game\", \"detail\": \"waived by carol: the incentive argument lives in the chain narrative: C-0002 pays the operator pe\", \"result\": \"pass\"}], \"submission_ready\": true}",
...
//
// morph §7.2: the adversarial-game entry pins the remediation that now
// quotes --strongest-attacker (the command must stay executable).
var explainGolden = map[string]string{
	"security-confirmed":         "{\"check\": \"security-confirmed\", \"gate\": \"bounty\", \"remediation\": \"webv2 verdict <campaign> <fid> --verdict confirmed --reason '<reasoning>' + webv2 recall <campaign> --finding <fid> + webv2 mint <fid> --exec <EXEC>  (see `webv2 gate explain` for the full CONFIRMED checklist)\"}",
	...
	"exploit-contract":           "{\"check\": \"exploit-contract\", \"gate\": \"bounty\", \"remediation\": \"webv2 exec <campaign> --profile docker-networkless --command 'forge test --match-test test_exploit' --finding <fid>  then webv2 mint <fid> --exec <EXEC-ID> --type foundry-test   (the program asks for a contract a triager can run, not a trace)\"}",
	...
	"adversarial-game":           "{\"check\": \"adversarial-game\", \"gate\": \"bounty\", \"remediation\": \"webv2 adversarial-game <campaign> <fid> --who-profit 'who profits from the freeze' --mechanism 'how the profit works' --interplay 'why the challenge path does not undo it' --strongest-attacker 'does a proof-valid bad batch still win the challenge?'   (each field >= 20 chars; or: webv2 waive <campaign> adversarial-game --subject <fid> --reason '...' if the incentive argument lives elsewhere, e.g. the chain narrative)\"}",
	"critic-verdict":             "{\"check\": \"critic-verdict\", \"gate\": \"confirmed\", \"remediation\": \"webv2 verdict <campaign> <fid> --verdict confirmed --reason '<reasoning>'\"}",
...
var bountyRemediationOrder = []string{"security-confirmed", "snapshot-pinned", "in-scope", "known-issue-check", "severity-floor", "evidence-sufficient", "fork-repro", "economic-quantified", "exploit-contract", "maximal-exploitation", "e7-price-basis", "claim-drift", "precondition-audit", "mainnet-fork-poc", "immunization", "accepted-risk", "paid-exploitability", "adversarial-game"}

// ---- unknown-check error (str(KeyError(msg))) ----
const wantUnknownCheck = "\"unknown check id 'no-such-check' (known: ['accepted-risk', 'adversarial-game', 'claim-drift', 'critic-verdict', 'e7-price-basis', 'economic-quantified', 'evidence-floor', 'evidence-floor-unreachable', 'evidence-sufficient', 'exploit-contract', 'fork-repro', 'immunization', 'in-scope', 'invariant-unverified', 'known-issue-check', 'mainnet-fork-poc', 'maximal-exploitation', 'memory-check', 'paid-exploitability', 'precondition-audit', 'reproduction-reproduced', 'reproduction-tier', 'security-confirmed', 'sequence-coverage', 'severity-floor', 'shield-adjudication', 'snapshot-compatible', 'snapshot-pinned'])\""

// ---- confirmed gate remediation (findings.GATE_REMEDIATION) ----
var confirmedRemediationGolden = map[string]string{
	"critic-verdict":             "webv2 verdict <campaign> <fid> --verdict confirmed --reason '<reasoning>'",
