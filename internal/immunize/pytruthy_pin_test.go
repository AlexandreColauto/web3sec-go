...
// pyTruthyLenientContainers pin (Wave J Task 7): records the truthiness this definition was
// already computing BEFORE the consolidation, so the unify/rename step cannot
// move a gate. Variant: pyTruthyLenientContainers — arrays/objects read truthy even when empty.
...
func TestPyTruthyPin(t *testing.T) {
	cases := []struct {
		...
		{"arr_empty", validation.VArr(), true},
		{"arr_zero", validation.VArr(validation.VInt(0)), true},
		{"obj_empty", validation.VObj(), true},
		...
		{"big_zero_literal", validation.VBigInt("0"), true},
	}
	for _, tc := range cases {
		if got := pyTruthyLenientContainers(tc.v); got != tc.want {
			t.Errorf("pyTruthyLenientContainers(%s) = %v, want %v", tc.name, got, tc.want)
		}
	}
}
