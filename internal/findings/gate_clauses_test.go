...
// Python twin (tests/test_gate_checklist.py) rather than hand-edited. The one
// deliberate divergence: remediation entries that name the campaign were
// re-recorded with the id substituted and then masked to <CAMPAIGN>, exactly
// as the messages already were — the gate holds the campaign, so a printed fix
// line carries the id instead of the <campaign> metavariable.
...
func TestClauseCollectorIsTheOnlySource(t *testing.T) {
	...
	// every clause carries its remediation from GATE_REMEDIATION, rendered
	// for the campaign in hand
	for _, cl := range clauses {
		if cl.Remediation != NameCampaign(GATE_REMEDIATION[cl.CheckID],
			c.CampaignID) {
