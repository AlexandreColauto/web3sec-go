...
| dependency scan | `scripts/security-check.sh` | govulncheck: nothing this code calls is a known vulnerability; a missing scanner or findings is INCOMPLETE/failed, never PASS — and every run prints the advisory-DB provenance it used (`advisory DB <source> as-of <timestamp>`, or an explicit "provenance unavailable"), so a PASS is dated to the DB it came from |
